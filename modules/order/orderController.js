...
const ORDER_STATUS = require('../../schema/orderSchema').ORDER_STATUS;
const orderController = {};

orderController.getAllOrderList = async (req, res, next) => {
  try {
    const { id, customer_id, user_id, warehouse,  confirmation ,returnOrder, search } = req.query;
    ...
    if(warehouse){
      searchQuery.is_confirmed = true;
    }

    if(confirmation){
       searchQuery.is_confirmed = false;
       searchQuery.order_type = "confirm";
    }
...
orderController.ChangeStatus = async(req, res,next) =>{
  ...
  try{
  const { orderId, status, tracking_number, name } = req.query;

   if (!orderId)  otherHelper.sendResponse( res, httpStatus.BAD_REQUEST,  false,null,  null,  'Order ID is required', null );
  
   if (!status)   otherHelper.sendResponse(res, httpStatus.BAD_REQUEST, false, null, null, 'Order status is required', null);


    if (!ORDER_STATUS || typeof ORDER_STATUS !== 'object') {
      return otherHelper.sendResponse(res, httpStatus.INTERNAL_SERVER_ERROR, false, null, null, 'Order status constants not available', null);
    }
    const validStatuses = Object.values(ORDER_STATUS);
    if (!validStatuses.includes(status)) {
      return otherHelper.sendResponse(res, httpStatus.BAD_REQUEST, false, null, null, 'Invalid order status', null);
    }

    const inTransitValue = ORDER_STATUS.IN_TRANSIT || 'in transit';
    if (status === inTransitValue && (!tracking_number || !name)) {
      return otherHelper.sendResponse(res, httpStatus.BAD_REQUEST, false, null, null, 'Tracking number and delivery name are required when order is in transit', null);
    }

   const updatedOrder = await orderSch.findOneAndUpdate(
      { _id: orderId }, 
      { $set: { is_confirmed: true, status: status, tracking_number: tracking_number, delivery_by: name } },
      { new: true } // return updated data
    );

    if (!updatedOrder)  otherHelper.sendResponse( res, httpStatus.BAD_REQUEST,  false,null,  null,  'Order not found or already confirmed', null );
...
orderController.getOrderById = async (req, res, next) => {
  try {
    ...
    const order = await orderSch.findOne({ order_id: id }).populate('products customer sales_executive');
    if (!order) {
      return otherHelper.sendResponse(res, httpStatus.NOT_FOUND, false, null, null, 'Order not found', null);
    }
