...
productController.getAllProductList = async (req, res, next) => {
  try {
    ...
    if (req.query.is_active !== undefined) {
      searchQuery.is_active = req.query.is_active === 'true';
    }
    ...
    if (req.query.id) {
      const user = await productSch.findById({ _id : req.query.id, is_active: true}).populate(populatedata);
      if (!user)  return otherHelper.sendResponse(res, httpStatus.NOT_FOUND, false, null, null, 'Product not found', null);
      
      const similarProduct = await productSch.find({ categories: user.categories, is_active: true }).populate(populatedata).limit(8);
      const responseData = {
        ...user.toObject(),
    ...
    if (req.query.category) {
      const categoryWiseProducts = await productSch.find({ categories: req.query.category, is_active :true }).populate(populatedata);
      if (!categoryWiseProducts)  return otherHelper.sendResponse(res, httpStatus.NOT_FOUND, false, null, null, 'Product not found', null);
    ...
    if (req.query.search && req.query.search !== 'null') {
      ...
      const companyIds = await companySch.find({ $or: [{ name_eng: regex }, { name_guj: regex }],  }) .select('_id');

      let searchResults = await productSch
        .find({ is_active : true, $or: [{ 'name.englishname': regex }, { 'name.gujaratiname': regex }, { 'tech_name.english_tech_name': regex }, { 'tech_name.gujarati_tech_name': regex }, { company: { $in: companyIds.map((c) => c._id) } }, { categories: { $in: categoryIds.map((c) => c._id) } }], })
        .populate(populatedata).skip((page-1)*size).limit(size)
        .exec();

        searchResults.totalData = await productSch.countDocuments({
           is_active: true,
    ...
    if (req.query.crop) {
      ...
      const searchResults = await productSch.find({ crops: req.query.crop,  is_active :true }).populate(populatedata).exec();
...
productController.AddProductData = async (req, res, next) => {
  try {
    ...
    if (Array.isArray(req.files) && req.files.length > 0) {
      Product.product_pics = req.files
        .filter((file) => file && (file.location || file.filename || file.path))
        .map((file) => file.location || file.filename || file.path);
    }
...
productController.DeleteProductData = async (req, res, next) => {
  try {
    ...
    let changeStatus = !Product.is_active;

    const deleted = await productSch.findByIdAndUpdate(id, { is_active: changeStatus}, {new: true});
    return otherHelper.sendResponse(res, httpStatus.OK, true, deleted, null, 'Product Staus Updated successfully', null);
  } catch (err) {
    next(err);
  }
};
