...
loginLogController.logout = async (req, res, next) => {
  try {
    ...
    let loginLog = await loginLogSch.findOne({ token });

    if (loginLog) {
      const time = moment();  
      const logoutDate = time.toDate(); 
      const loginDate = moment(loginLog.login_date);  
      const durationInMillis = logoutDate - loginDate;
      const durationInMinutes = Math.floor(durationInMillis / (1000 * 60));
      const durationInHours = Math.floor(durationInMinutes / 60);

      const formattedHours = String(durationInHours).padStart(2, '0');
      const formattedMinutes = String(durationInMinutes % 60).padStart(2, '0');
      const formattedDuration = `${formattedHours}:${formattedMinutes}`;

      const updatedLoginLog = await loginLogSch.findOneAndUpdate(
        { token },
        {
          $set: {
            is_active: false,
            logout_date: logoutDate,
            Login_duration: formattedDuration
          }
        },
        { new: true }
      );
      return otherHelper.sendResponse(res, httpStatus.OK, true, null, null, 'Logged out', updatedLoginLog);
    } else {
