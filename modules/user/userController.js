...
userController.UpdateUserImage = async (req, res, next) => {
  try {
    ...
    if (req?.files && req.files.length > 0) {
      updateData.$set.user_pic = req.files[0].location;
    }
...
userController.updateUserProfileImage = async (req, res, next) => {
  try {
    ...
    if (!req?.files || req.files.length === 0)    return otherHelper.sendResponse(res, httpStatus.BAD_REQUEST, false, null, null, 'Profile image file is required', null);
    ...
    const updateData = {$set: {  user_pic: req.files[0].location,  updated_at: new Date()  } };
    const updatedUser = await userSch.findByIdAndUpdate(userId, updateData, { new: true });
